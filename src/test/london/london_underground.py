...
def get_station_by_name(stations, name):
	name = name.lower()
	ratios = [(SequenceMatcher(None, name, v.name.lower()).ratio(),v) for v in stations.values()]
...
def get_path(s1, s2):
	def distance(n1, n2):
		...
		latB, longB = n2.position
		# convert degres to radians!!
		latA, latB, longA, longB = map(lambda d: d * math.pi / 180, (latA, latB, longA, longB))
		x = (longB - longA) * math.cos((latA + latB) / 2)
		y = latB - latA
		return math.hypot(x, y)
...
if __name__ == '__main__':
	if len(sys.argv) != 3:
		print('Usage : {script} <station1> <station2>'.format(script=sys.argv[0]))
		sys.exit(1)
